...
int main(int argc, char *argv[]) {
	...
	puts("[DEBUG] Initialising sniffer...\n");
	thread_handle = CreateThread(NULL, 0, init_sniffer_thread, address, 0, &thread_id);
	...
	/* thread for reading incoming messages from server */
	thread_handle = CreateThread(NULL, 0, read_msg_thread, &server_fd, 0, &thread_id);
	CloseHandle(thread_handle);
